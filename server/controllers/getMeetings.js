  ...
  const getMeetings = async (req, res) => {
    ...
    try {
      ...
      // Fetch original meeting time and timezone
      const [results] = await connection.query(
        'SELECT original_meeting_time, original_meeting_date, original_timezone FROM meeting_requests WHERE DATE(original_meeting_date) = ?',
        [date]
      );
